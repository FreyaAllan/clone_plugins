    ...
    if message.chat.type == enums.ChatType.PRIVATE:
        ...
        if len(message.command) == 2 and message.command[1] == 'help':
            ...
            if settings['shortlink']:
                link = await get_shortlink(int(group_id), f"https://telegram.me/{clonedme.U_NAME}?start=shortlink_{base64_string}")
                btn = [[
                    InlineKeyboardButton("✨ ᴅᴏᴡɴʟᴏᴀᴅ ✨", url=link)
                ]]
                await message.reply(f"<code>{file['file_name']}</code>\n\n<b>ʏᴏᴜʀ ꜰɪʟᴇ ɪs ʀᴇᴀᴅʏ 💥,\nᴘʟᴇᴀsᴇ ᴅᴏᴡɴʟᴏᴀᴅ ᴜsɪɴɢ ᴛʜɪs ʟɪɴᴋ...</b>", reply_markup=InlineKeyboardMarkup(btn), protect_content=True)
            else:
                btn = [[
                        InlineKeyboardButton("‼️ ᴄʟᴏsᴇ ‼️", callback_data=f'close#{message.from_user.id}')
                    ]]
            ...
            buttons = [[
                InlineKeyboardButton('+ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ +', url=f'http://t.me/{clonedme.U_NAME}?startgroup=start')
            ],[
        ...
        if message.chat.id == SUPPORT_GROUP:
            return await message.reply("<b>ʏᴇᴘ, ᴛʜɪs ɪs sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ!! ʜᴏᴡ ᴄᴀɴ ʜᴇʟᴘ ʏᴏᴜ? 😊</b>")
