...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(script.START_TXT.format(message.from_user.mention if message.from_user else message.chat.title, clonedme.U_NAME, clonedme.B_NAME), reply_markup=reply_markup, disable_web_page_preview=True)
    ...
    if data.startswith("sendfiles"):
        ...
        g = await get_shortlink(chat_id, f"https://telegram.me/{clonedme.U_NAME}?start=allfiles_{file_id}")
        ...
        files = files_[0]
        g = await get_shortlink(chat_id, f"https://telegram.me/{clonedme.U_NAME}?start=file_{file_id}")
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{clonedme.U_NAME}?start="))
                ]]
                await message.reply_text(
                ...
                return
            await client.send_cached_media.copy(int(message.from_user.id))
    ...
    if not files_:
        ...
        try:
            ...
            title = f'{clonedme.B_NAME} ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_[0]
    title = f'{clonedme.B_NAME} ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"{clonedme.B_NAME} {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{clonedme.U_NAME}?start="))
        ]]
...
async def handle_requests(bot, message):
    ...
    try:
        if LOG_CHANNEL is not None and len(content) >= 3:
            ...
            reported_post = await clnapp.send_message(chat_id=LOG_CHANNEL, text=f"<b>𝖱𝖾𝗉𝗈𝗋𝗍𝖾𝗋 : {mention} ({reporter})\n\n𝖬𝖾𝗌𝗌𝖺𝗀𝖾 : {content}</b>", reply_markup=InlineKeyboardMarkup(btn))
            success = True
        else:
            if len(content) < 3:
    ...
    if success:
        bot = clonedme.U_NAME
        btn = [[
                InlineKeyboardButton("🧧 VIEW STATUS 🧧", url=f"https://telegram.me/{bot}")
               ]]
